import json  # for end_to_end.ipynb
import os
import subprocess
import sys
...
def main(args):  # noqa: WPS210
    ...
    nnotes_path = str(args[7])
    parameters['generation_params']['additional_features_params']['nnotes']['preset'] = json.load(open(nnotes_path, 'r'))
    pprint(parameters['generation_params']['additional_features_params']['nnotes']['preset'][:4])
    # kvm nnotes:
    ...
    subprocess.run(
        ['/storage/arkady/miniconda3/envs/transformer_production/bin/python',
         '/storage/arkady/Glinka/music-transformer/video_conditioning/code3/my_generate_sample.py', 
         output_dir,
         output_name
        ],
    )
