...
def main(args=None):  # noqa: WPS213, WPS210
    ...
    """
    # /storage/arkady/Glinka/music-transformer/video_conditioning/code3/model/model_params.json
    # with open('../model/model_params.json', 'r') as model_params_file:
    with open('/storage/arkady/Glinka/music-transformer/video_conditioning/code3/model/model_params.json', 'r') as model_params_file:
    ...
    if args is not None and len(args) > 2:
        ...
        with open(  # noqa: WPS440
            'generate_sample_parameters.json',
            'r',
        ) as json_file:
            params = json.load(json_file)  # noqa: WPS110

    print('=====generate_samples======', os.path.abspath('.'))
    # pprint(params[:4])
    ...
    model = MusicTransformer(  # можно переписать на импорт из music-transformer
        **model_params,
    )
...
def set_nnotes(afg: GenerationAdditionalFeatures):
    ...
    with open('/data/transformer-production/code3/presets/genre_timings.json', 'r') as fp:
...
def save_generated(
    ...
    if save_params:
        with open(os.path.join(folder, f'{output_folder_name}_params.json'), 'w') as json_output:
            with open(  # noqa: WPS440
                '/storage/arkady/Glinka/music_generation/glinka_server/code3/generation/generate_sample_parameters.json',
                'r',
            ) as json_file:
                params = json.load(json_file)  # noqa: WPS110
