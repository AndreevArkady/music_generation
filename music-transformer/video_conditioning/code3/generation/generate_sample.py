...
def main(args=None):  # noqa: WPS213, WPS210
    ...
    gets config from '{0}/generate_sample_{1}_parameters.json' from where executed
    """
    ...
    if len(args) == 1:
        path_parameters = 'generation/generate_sample_parameters.json'
    elif len(args) == 2:
        path_parameters = args[1]
    elif len(args) >= 3:
        path_parameters ='{0}/generate_sample_{1}_parameters.json'.format(
            args[1],
            args[2],
        )
    assert path_parameters is not None
    with open(  # noqa: WPS440
        path_parameters,
        'r',
    ) as json_file:
    ...
    print('---- save_generated')
    pprint(params['saving_options'])
    save_generated(
        generated_sequence,
        primary,
...
def save_generated(
    ...
    save_primary: bool,
    save_mp3: bool,
    path_parameters: str = 'generation/generate_sample_parameters.json'
):
